...
class DB {
  ...
  public static function getMDB() {
    ...
    //if so we need to create a new connection
    $db_class_vars = get_class_vars('DB'); // the DB::$$var syntax only works in 5.3+

    static $reconnect_variables = array('user', 'password', 'host', 'port', 'encoding');
    $reconnect = FALSE;
    foreach ($reconnect_variables as $r) {
      if ($mdb->$r !== $db_class_vars[$r]) {
        $reconnect = TRUE;
        break;
      }
    }
